...
function gameLoop(timestamp) {  // Game loop and also the heart of the game. Runs based on deltaTime to ensure smooth movement
    const deltaTime = (timestamp - global.lastTime) / 1000; // Time difference in seconds
    global.lastTime = timestamp;

    // Update player
    player.update(deltaTime); // Method inside the Player class to update the player's position and velocity, check for collisions, and update the score
